...
class DockerConfigForm(BaseForm):
    id = HiddenField()
    host = StringField(
        "Docker Host", description="느그 도커 API 엔드포인트"
    )
    hostname = StringField(
        "Docker Hostname", description="외부에 보여줄 느그 도커 URL"
    )
...
def create_container(docker, image, team, portbl):
    ...
    print("needed ports", needed_ports)
    team = hashlib.md5(team.encode("utf-8")).hexdigest()[:10]
    container_name = "%s_%s_%s" % (image.split(':')[1], team, random.randint(10000, 99999))
    assigned_ports = dict()
