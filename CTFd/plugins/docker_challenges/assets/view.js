        ...
        $.each(result['data'], function (i, item) {
            if (item.docker_image == container) {
                $('#docker_container').html(`
                    <div>
                    <h4 class="docker-title">컨테이너 접속 정보</h4>
                    <p class="docker-connections">
                        Host: <code>${item.host}</code>
                        ${item.ports.map(port => `
                                <br>
                                Port: <code>${port}</code>
                        `).join('')}
                ...
                var x = setInterval(function () {
                    ...
                    $(`#${String(item.instance_id).substring(0, 10)}_revert_container`).html(`다음 컨테이너 재시작까지 ${minutes}:${seconds}`);
                    if (distance < 0) {
                        clearInterval(x);
                        $(`#${String(item.instance_id).substring(0, 10)}_revert_container`)
                            .html(`
                                <a onclick="start_container('${item.docker_image}');" class='btn btn-primary'>
                                    <small style='color:white;'><i class="fas fa-redo"></i> 컨테이너 재시작</small>
                                </a>
                            `);
                    }
                }, 1000);
